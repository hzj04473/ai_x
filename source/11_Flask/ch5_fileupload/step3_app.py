# step3 : 파일첨부화면 + 첨부 했던 파일 목록 (다운로드 + 삭제)
...
from flask import redirect # 삭제 후 '/' 요청경로로 redirect
...
def index():
    ...
    if form.validate_on_submit(): # 폼 데이터가 유효한지 (POST요청이 유효하게 들어왔는지) 체크
        ...
        infos = [] # 파일 정보 (파일명, 생성시간, 파일최종수정시점, 사이즈) 리스트
        
        for filename in filelist:
            ctime, mtime, atime, size = info(filename)
            fileinfo = {'name':filename, 'ctime':ctime, 'mtime':mtime, 'atime':atime, 'size':size}
...
def download(filename):
    return send_file(UPLOAD_FOLDER + filename, 
                    as_attachment=True) # 브라우저에서 파일이 얼리지 않고 다운로드만 가능
